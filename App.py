...
# Fungsi Anti-Gagal untuk Menampilkan Foto (Format Lama & Baru)
def tampilkan_foto(url_data, caption=""):
    ...
    if isinstance(url_data, str) and url_data.startswith("http"):
        ...
        cols = st.columns(len(url_data) if len(url_data) < 4 else 3)
        for idx, u in enumerate(url_data):
            if isinstance(u, str) and u.startswith("http"):
                cols[idx%3].image(u, caption=caption, width=200)
...
if menu == "📝 Form Pengecekan":
    ...
    with tabs[6]:
        ...
        if st.button("🚀 Upload & Submit", type="primary"):
            if not site_name: st.error("Isi Nama Site!")
            else:
                with st.spinner("⏳ Menyimpan data & foto..."):
                    p_res = []
                    for p in panel_data:
...
# =========================================================================
# MENU 2: HASIL PENGECEKAN (TAMPILAN PASTINYA MUNCUL & BISA NAMBAH FOTO BEBAS)
# =========================================================================
    ...
    if not db:
        ...
        for i in range(len(db) - 1, -1, -1):
            ...
            with st.expander(f"📍 {r.get('site_name', 'Unknown')} | {r.get('timestamp', '')} | Status: {r.get('status', '')}"):
                ...
                # --- TAMPILAN UTAMA (SELALU MUNCUL, TIDAK DISEMBUNYIKAN) ---
                ...
                st.divider()
                # --- FITUR EDIT & UPLOAD TAMBAHAN BEBAS (DI BAWAH) ---
                ...
                with st.container(border=True):
                    ...
                    st.markdown("**Tambah Foto Susulan (Bebas Foto Apapun Sesuai Kategori):**")
                    up_f = st.file_uploader("Fisik / Shading", accept_multiple_files=True, key=f"uf_{i}")
                    up_p = st.file_uploader("Panel Surya (Bebas)", accept_multiple_files=True, key=f"up_{i}")
                    up_b = st.file_uploader("Baterai (Bebas)", accept_multiple_files=True, key=f"ub_{i}")
                    up_e = st.file_uploader("Elektrikal (SCC/JB)", accept_multiple_files=True, key=f"ue_{i}")
